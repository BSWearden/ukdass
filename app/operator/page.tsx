  ...
  const { data: permissionData, error: permissionError } = await supabase
    ...
    .select(`
      ...
      danger_areas (
        ...
        authority, airspace_when_inactive, current_status, status_updated_at
      )
    `)
    .eq('user_id', claims.sub)
            ...
            return (
              <article key={area.id} style={{background:'linear-gradient(180deg,#0b1722,#08131c)',border:'1px solid #203243',borderRadius:'14px',padding:'20px',boxShadow:'0 16px 40px rgba(0,0,0,.20)'}}>
                <div style={{display:'flex',justifyContent:'space-between',gap:'12px',alignItems:'flex-start'}}>
                  ...
                  <span style={{...badge,padding:'6px 9px',borderRadius:'999px',fontSize:'11px',fontWeight:900,letterSpacing:'.08em'}}>{area.current_status}</span>
                </div>
                ...
                <div style={{marginTop:'15px',borderTop:'1px solid #203243',paddingTop:'13px'}}>
                  <div style={{fontSize:'10px',color:'#7892a4',textTransform:'uppercase',letterSpacing:'.12em',fontWeight:800}}>Last DASS status update</div>

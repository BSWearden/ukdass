...
type OperatorProfile={user_id:string;display_name:string;organisation_id:string|null;account_status:string;must_change_password:boolean;created_at:string;updated_at:string}
...
type AdminAudit={id:number;admin_user_id:string;action_type:string;target_user_id:string|null;danger_area_id:string|null;summary:string;created_at:string}

function utc(value:string|null){
  if(!value)return'—'
  return new Intl.DateTimeFormat('en-GB',{timeZone:'UTC',day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).format(new Date(value))+' UTC'
}
function badge(status:string){
  if(status==='ACTIVE'||status==='SENT'||status==='ACKNOWLEDGED')return{color:'#84e8b0',background:'rgba(79,209,139,.10)',border:'rgba(79,209,139,.34)'}
  if(status==='UNVERIFIED'||status==='FAILED'||status==='SUSPENDED')return{color:'#ff9299',background:'rgba(255,90,100,.10)',border:'rgba(255,90,100,.34)'}
...
export default async function AdminPage(){
  ...
  const [operatorResult,organisationResult,permissionResult,areaResult,eventResult,notificationResult,auditResult]=await Promise.all([
    supabase.from('operator_profiles').select('user_id,display_name,organisation_id,account_status,must_change_password,created_at,updated_at').order('display_name',{ascending:true}),
    ...
    supabase.from('status_events').select('id,danger_area_id,previous_status,new_status,changed_by,changed_at,note,event_source,event_type').order('changed_at',{ascending:false}).limit(30),
    ...
    supabase.from('admin_audit_log').select('id,admin_user_id,action_type,target_user_id,danger_area_id,summary,created_at').order('created_at',{ascending:false}).limit(30),
  ])
  ...
  return <main style={{minHeight:'100vh',background:'#071019',color:'#edf5fb',padding:'clamp(14px,3vw,26px)'}}>
    <div style={{maxWidth:'1280px',margin:'0 auto'}}>
      <header style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:'16px',flexWrap:'wrap',paddingBottom:'18px',borderBottom:'1px solid #203243'}}>
        <div><div style={{fontSize:'10px',letterSpacing:'.16em',textTransform:'uppercase',color:'#7f9db0',fontWeight:850}}>DASS Alpha 0.5.1 · Administration & Governance</div><h1 style={{margin:'5px 0 4px',fontSize:'clamp(25px,5vw,32px)'}}>Administrator Dashboard</h1><div style={{fontSize:'13px',color:'#91a6b8'}}>{adminProfile.display_name} · {adminProfile.admin_role}</div></div>
      ...
      <AdminOperatorControls organisations={organisations} areas={areas.map(a=>({id:a.id,code:a.code,name:a.name}))} operators={operators.map(o=>({user_id:o.user_id,display_name:o.display_name,organisation_id:o.organisation_id,account_status:o.account_status,must_change_password:o.must_change_password}))} permissions={permissions}/>

      <section style={{marginTop:'26px'}}><Heading eyebrow="Operational oversight" title="Danger Area State"/><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:'10px'}}>
        {areas.map(a=>{const s=badge(a.current_status);return <article key={a.id} style={card}><div style={{display:'flex',justifyContent:'space-between',gap:'10px'}}><div><strong style={{color:'#8fdaf0'}}>{a.code}</strong><div style={{marginTop:'3px',fontSize:'11px',color:'#91a6b8'}}>{a.name}</div></div><span style={{fontSize:'9px',fontWeight:900,padding:'5px 8px',borderRadius:'999px',color:s.color,background:s.background,border:`1px solid ${s.border}`}}>{a.current_status}</span></div><div style={{marginTop:'12px',display:'grid',gridTemplateColumns:'1fr 1fr',gap:'7px'}}><Data label="Last status update" value={utc(a.status_updated_at)}/><Data label="Valid until" value={utc(a.status_valid_until)}/><Data label="Window opens" value={utc(a.reporting_window_start_at)}/><Data label="Window closes" value={utc(a.reporting_window_end_at)}/><Data label="Scheduled activation" value={utc(a.scheduled_activation_at)}/><Data label="Organisation" value={a.organisation_id?orgMap.get(a.organisation_id)?.name??'Unknown':'—'}/></div></article>})}
      </div></section>

      <section style={{marginTop:'26px'}}><Heading eyebrow="System activity" title="Recent Status Events"/><div style={listBox}>{events.length===0?<Empty text="No status events recorded."/>:events.map((e,i)=><div key={e.id} style={{padding:'11px 13px',borderTop:i?'1px solid #182b38':0}}><div style={{display:'flex',justifyContent:'space-between',gap:'10px',flexWrap:'wrap'}}><strong style={{fontSize:'11px'}}>{areaMap.get(e.danger_area_id)?.code??'Unknown DA'} · {e.event_type}</strong><span style={{fontSize:'9px',color:e.event_source==='SYSTEM'?'#fbbf24':'#8fdaf0',fontWeight:900}}>{e.event_source}</span></div><div style={{marginTop:'4px',fontSize:'10px',color:'#91a6b8'}}>{e.previous_status??'—'} → {e.new_status??'—'} · {utc(e.changed_at)}</div><div style={{marginTop:'3px',fontSize:'9px',color:'#607888'}}>Actor: {e.changed_by?operatorMap.get(e.changed_by)?.display_name??e.changed_by:'DASS system'}</div></div>)}</div></section>

      <section style={{marginTop:'26px'}}><Heading eyebrow="Notification assurance" title="Recent Operational Notifications"/><div style={listBox}>{notifications.length===0?<Empty text="No operational notifications recorded."/>:notifications.map((n,i)=>{const state=n.acknowledged_at?'ACKNOWLEDGED':n.seen_at?'SEEN':n.status;const s=badge(state);return <div key={n.id} style={{padding:'11px 13px',borderTop:i?'1px solid #182b38':0}}><div style={{display:'flex',justifyContent:'space-between',gap:'10px'}}><strong style={{fontSize:'11px'}}>{areaMap.get(n.danger_area_id)?.code??'Unknown DA'} · {n.notification_type}</strong><span style={{fontSize:'9px',fontWeight:900,color:s.color}}>{state}</span></div><div style={{marginTop:'4px',fontSize:'10px',color:'#91a6b8'}}>{operatorMap.get(n.user_id)?.display_name??'Unknown operator'} · Attempts {n.attempts} · {utc(n.created_at)}</div></div>})}</div></section>

      <section style={{marginTop:'26px'}}><Heading eyebrow="Administrative accountability" title="Admin Audit Log"/><div style={listBox}>{audits.length===0?<Empty text="No administrative write actions recorded."/>:audits.map((a,i)=><div key={a.id} style={{padding:'11px 13px',borderTop:i?'1px solid #182b38':0}}><strong style={{fontSize:'11px'}}>{a.action_type}</strong><div style={{marginTop:'4px',fontSize:'10px',color:'#91a6b8'}}>{a.summary}</div><div style={{marginTop:'3px',fontSize:'9px',color:'#607888'}}>{utc(a.created_at)}</div></div>)}</div></section>

      <section style={{marginTop:'22px',borderLeft:'3px solid #d97706',background:'rgba(217,119,6,.055)',padding:'11px 13px',fontSize:'10px',lineHeight:1.5,color:'#cfbf9d'}}><strong>Governance boundary:</strong> Administrator account-management capability does not confer authority to ACTIVATE, STAND DOWN or schedule a Danger Area.</section>
    </div>
  </main>
}

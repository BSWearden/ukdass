-- UK DASS Alpha 1.4.0: retain and display activation-specific NOTAM vertical limits.
-- The AIP limits remain the structural envelope; current/upcoming NOTAM limits
-- become the effective displayed limits while that activation is published.

alter table public.danger_area_notams
  add column if not exists lower_limit text,
  add column if not exists upper_limit text;

update public.danger_area_notams n
set lower_limit=i.lower_limit,
    upper_limit=i.upper_limit
from public.notam_import_items i
where i.import_run_id=n.sync_run_id
  and i.notam_number=n.notam_number
  and i.danger_area_id=n.danger_area_id
  and i.valid_from=n.valid_from
  and (i.lower_limit is not null or i.upper_limit is not null)
  and (n.lower_limit is distinct from i.lower_limit or n.upper_limit is distinct from i.upper_limit);
...
begin
 ...
 update public.danger_areas set aip_current=false where airspace_source='TDA_NOTAM' and temporary_valid_until is not null and temporary_valid_until<=now();
 ...
 from public.notam_import_items i where i.import_run_id=p_import_run_id and i.event_kind='ACTIVATION' and i.schedule_status='RESOLVED' and i.match_status='UNMATCHED' and i.designator is not null and jsonb_typeof(i.parsed_geometry)='array' and jsonb_array_length(i.parsed_geometry)>=3
 ...
 update public.notam_import_items i set danger_area_id=d.id,match_status='MATCHED',included=true from public.danger_areas d where i.import_run_id=p_import_run_id and i.event_kind='ACTIVATION' and i.schedule_status='RESOLVED' and i.match_status='UNMATCHED' and d.code=i.designator and d.airspace_source='TDA_NOTAM' and d.aip_current;

 update public.danger_area_notams n set lifecycle_status='CANCELLED',last_seen_at=now()
 where n.source_name='NATS_PIB' and n.lifecycle_status='ACTIVE' and n.valid_from<v_run.coverage_end and coalesce(n.valid_until,'infinity'::timestamptz)>v_run.coverage_start
  and not exists(select 1 from public.notam_import_items i where i.import_run_id=p_import_run_id and i.included and i.event_kind='ACTIVATION' and i.match_status='MATCHED' and i.schedule_status='RESOLVED' and i.notam_number=n.notam_number and i.danger_area_id=n.danger_area_id and i.valid_from=n.valid_from);
 get diagnostics v_retired=row_count;

 insert into public.danger_area_notams(notam_number,danger_area_id,matched_designator,valid_from,valid_until,lower_limit,upper_limit,lifecycle_status,match_status,match_method,match_confidence,q_code,raw_text,source_name,sync_run_id,last_seen_at)
 select i.notam_number,i.danger_area_id,i.designator,i.valid_from,i.valid_until,nullif(i.lower_limit,''),nullif(i.upper_limit,''),'ACTIVE','MATCHED',case when d.airspace_source='TDA_NOTAM' then 'NATS_PIB_NOTAM_POLYGON' when i.resolved_at is null then 'NATS_PIB_QCODE_DESIGNATOR' else 'NATS_PIB_ADMIN_SCHEDULE_RESOLUTION' end,1.0,i.q_code,i.raw_text,'NATS_PIB',p_import_run_id,now()
 ...
 on conflict(notam_number,danger_area_id,valid_from) do update set matched_designator=excluded.matched_designator,valid_until=excluded.valid_until,lower_limit=excluded.lower_limit,upper_limit=excluded.upper_limit,lifecycle_status='ACTIVE',match_status='MATCHED',match_method=excluded.match_method,match_confidence=excluded.match_confidence,q_code=excluded.q_code,raw_text=excluded.raw_text,source_name='NATS_PIB',sync_run_id=excluded.sync_run_id,last_seen_at=now();
 get diagnostics v_count=row_count;

 update public.danger_areas d set aip_current=false where d.airspace_source='TDA_NOTAM' and not exists(select 1 from public.danger_area_notams n where n.danger_area_id=d.id and n.lifecycle_status='ACTIVE' and (n.valid_until is null or n.valid_until>now()));
...
select d.id as danger_area_id,d.code,(current_n.id is not null) as has_live_notam,
 ...
 case
  ...
  when s.source_name='NATS PIB manual import' and latest_pib.coverage_end is not null then case when latest_pib.coverage_end<=now() then 'STALE' else 'CURRENT' end
 ...
 published_n.lower_limit as notam_lower_limit,published_n.upper_limit as notam_upper_limit
from public.danger_areas d cross join public.notam_system_state s
left join lateral(select r.coverage_end from public.notam_sync_runs r where r.source_name='NATS_PIB' and r.sync_status='SUCCESS' and r.published_at is not null order by r.published_at desc,r.started_at desc limit 1) latest_pib on true
